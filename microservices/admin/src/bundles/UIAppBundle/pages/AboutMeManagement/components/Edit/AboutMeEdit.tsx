...
import { AboutMeEditForm } from "../../config/AboutMeEditForm";
...
export function AboutMeEdit(props: { id: string }) {
  const UIComponents = useUIComponents();
  const t = useTranslate();
  const form = use(AboutMeEditForm, { transient: true });
  ...
  const {
    ...
    AboutMeEditForm.getRequestBody()
  );
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.about_me.edit.header")}
        ...
        extra={
          features.view
            ? [
                <Link
                  key="view"
                  to={router.path(Routes.ABOUT_ME_VIEW, {
                    params: { id: props.id },
                  })}
                >
      ...
      <Ant.Card>
        ...
        {!isLoading && !error && (
          <Ant.Form
            ...
            initialValues={document as AboutMe}
