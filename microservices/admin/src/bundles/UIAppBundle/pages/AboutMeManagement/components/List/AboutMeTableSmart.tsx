...
import { AboutMeList } from "../../config/AboutMeList";

export class AboutMeAntTableSmart extends AntTableSmart<AboutMe> {
  collectionClass = AboutMeCollection;

  getBody(): QueryBodyType<AboutMe> {
    return AboutMeList.getRequestBody();
  }

  getColumns(): ColumnsType<AboutMe> {
    const list = this.container.get(AboutMeList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<AboutMe> {
    return {
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.ABOUT_ME_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.ABOUT_ME_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });
