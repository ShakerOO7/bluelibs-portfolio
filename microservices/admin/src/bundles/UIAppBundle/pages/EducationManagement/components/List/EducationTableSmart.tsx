...
import { EducationList } from "../../config/EducationList";

export class EducationAntTableSmart extends AntTableSmart<Education> {
  collectionClass = EducationCollection;

  getBody(): QueryBodyType<Education> {
    return EducationList.getRequestBody();
  }

  getColumns(): ColumnsType<Education> {
    const list = this.container.get(EducationList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<Education> {
    return {
  ...
  getSortMap() {
    return EducationList.getSortMap();
  }
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.EDUCATION_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.EDUCATION_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });
