...
import { ImagesAntTableSmart } from "./ImagesTableSmart";
...
export function ImagesList() {
  ...
  const [api, Provider] = newSmart(ImagesAntTableSmart);
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.images.list.header")}
        extra={[
          features.create ? (
            <Ant.Button
              key="1"
              onClick={() => router.go(Routes.IMAGES_CREATE)}
              icon={<PlusOutlined />}
            >
              {t("management.images.list.create_btn")}
            </Ant.Button>
          ) : null,
          <Ant.Button
            key="2"
      ...
      <Ant.Layout.Content>
        <Provider>
          <div className="page-images-list">
            {filtersOpened && <ImagesListFilters onUpdate={onFiltersUpdate} />}
            <Ant.Input.Search
              name="Search"
