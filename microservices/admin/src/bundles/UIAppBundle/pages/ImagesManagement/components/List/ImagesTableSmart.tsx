...
import { Image, ImagesCollection } from "@bundles/UIAppBundle/collections";
...
export class ImagesAntTableSmart extends AntTableSmart<Image> {
  collectionClass = ImagesCollection;

  getBody(): QueryBodyType<Image> {
    return ImageList.getRequestBody();
  }

  getColumns(): ColumnsType<Image> {
    const list = this.container.get(ImageList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<Image> {
    return {
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.IMAGES_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.IMAGES_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });
