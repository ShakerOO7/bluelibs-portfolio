...
import { ProjectsAntTableSmart } from "./ProjectsTableSmart";
...
export function ProjectsList() {
  ...
  const [api, Provider] = newSmart(ProjectsAntTableSmart);
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.projects.list.header")}
        extra={[
          features.create ? (
            <Ant.Button
              key="1"
              onClick={() => router.go(Routes.PROJECTS_CREATE)}
              icon={<PlusOutlined />}
            >
              {t("management.projects.list.create_btn")}
            </Ant.Button>
          ) : null,
          <Ant.Button
            key="2"
      ...
      <Ant.Layout.Content>
        <Provider>
          <div className="page-projects-list">
            {filtersOpened && (
              <ProjectsListFilters onUpdate={onFiltersUpdate} />
            )}
            <Ant.Input.Search
              name="Search"
