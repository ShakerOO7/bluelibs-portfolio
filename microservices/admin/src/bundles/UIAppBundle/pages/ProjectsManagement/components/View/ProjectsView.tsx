...
import { ProjectViewer } from "../../config/ProjectViewer";
import { features } from "../../config/features";

export function ProjectsView(props: { id: string }) {
  ...
  if (isLoading) {
    ...
    if (error || document === null) {
      ...
      content = <ProjectsViewComponent document={document} />;
    }
  }
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.projects.view.header")}
...
export function ProjectsViewComponent(props: { document: Partial<Project> }) {
  const document = props.document;

  const viewer = use(ProjectViewer, { transient: true });
...
export function getHeaderActions(
  ...
  if (features.edit) {
    actions.push(
      <Link
        key="edit"
        to={router.path(Routes.PROJECTS_EDIT, {
          params: { id },
        })}
      >
      ...
      <Ant.Popconfirm
        ...
        onConfirm={() => {
          collection.deleteOne(id).then(() => {
            router.go(Routes.PROJECTS_LIST);
  ...
  const viewRoutePath = router.path(Routes.PROJECTS_VIEW, {
    params: { id },
  });
